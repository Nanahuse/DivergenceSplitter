...
class TestReleaseClosure:
    def test_traces_transitive_dependencies(self) -> None:
        ...
        leaf = FakeDistribution("leaf", "2.0.0", license="MIT")
    ...
    def test_evaluates_environment_markers_for_windows(self) -> None:
        ...
        windows = FakeDistribution("windows-only", "1.0.0", license="MIT")
        mac = FakeDistribution("mac-only", "1.0.0", license="MIT")
        extra = FakeDistribution("extra-only", "1.0.0", license="MIT")
    ...
    def test_dev_and_build_dependencies_are_not_included(self) -> None:
        pytest_dist = FakeDistribution("pytest", "9.1.1", license="MIT")
        pyinstaller_dist = FakeDistribution(
            "pyinstaller", "6.22.2", license="GPL-2.0-or-later"
        )
...
class TestBuildInventory:
    def test_packages_are_sorted_by_normalized_name(self) -> None:
        dists = [
            ...
            FakeDistribution("opencv-python", "5.0.0.93", license="Apache 2.0"),
        ]
