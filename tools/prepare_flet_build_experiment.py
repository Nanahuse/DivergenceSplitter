...
def add_workspace_dev_packages() -> None:
    ...
    if block not in text:
        ...
        UI.joinpath("pyproject.toml").write_text(text.replace(marker, block + "\n" + marker), encoding="utf-8")


def replace_git_sources_with_pep508() -> None:
    text = RUNTIME.read_text(encoding="utf-8")
    text = text.replace('    "livesplit-bridge-client",', '    "livesplit-bridge-client @ git+https://github.com/Nanahuse/livesplit-bridge-client.git@v0.2.0",')
    text = text.replace('    "windows-capture-device-list; sys_platform == \'win32\'",', '    "windows-capture-device-list @ git+https://github.com/Nanahuse/windows-capture-device-list.git@v0.2.0; sys_platform == \'win32\'",')
    start = text.index("\n[tool.uv.sources]")
    end = text.index("\n[build-system]", start)
    RUNTIME.write_text(text[:start] + text[end:], encoding="utf-8")
    append_once(UI / "pyproject.toml", '[tool.flet.windows]\ndependencies = ["ndi-python>=6.3.2.4"]')
...
def main() -> None:
    ...
    parser.add_argument("mode", choices=["entry-only", "workspace-dev-packages", "pep508-git", "all-dev-packages"])
